...
namespace SecurityLibrary.DES
{
	...
	public class DES : CryptographicTechnique
	{
		...
		public string encrypt_decrypt(string Text, List<string> keys)
		{
			...
			int[] initial_perm = { 58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46,
			...
			for (int i = 0; i < 16; i++)
			{
				string right_expanded = permute(right, ExpansionTable);
			...
			int[] final_perm = { 40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6,
			...
			string txt = "0x" + Convert.ToInt64(permute(combinedkey, final_perm), 2).ToString("x").PadLeft(16, '0');
			return txt;
		}
		public override string Decrypt(string cipherText, string key)
		{
			...
			int[] PC_1 = { 57, 49, 41, 33, 25, 17, 9, 1, 58, 50, 42, 34, 26, 18, 10, 2, 59,
			...
			int[] PC_2 = { 14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26,
			...
			for (int i = 0; i < 16; i++)
			{
				...
				string combine = C + D;
				string RoundKey = permute(combine, PC_2);
				rkb.Add(RoundKey);
			...
			string plainText = encrypt_decrypt(cipherText, rkb);
			return plainText;
		...
		public override string Encrypt(string plainText, string key)
		{
			...
			for (int i = 0; i < 16; i++)
			{
				...
				D = shift_left(D, shift_table[i]);
				string K = C + D;
				string RoundKey = permute(K, PC_2);
				keys.Add(RoundKey);
